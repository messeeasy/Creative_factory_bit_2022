...
def parse_audio_files(data):
        ...
        for data_x in data:
            ...
            mfccs, chroma, mel= extract_feature(data_x,4000)
            ext_features = np.hstack([mfccs,chroma,mel])
            #features = np.vstack([features,ext_features]) 
            feature_data.append(ext_features)
        return feature_data
#%%
print(len(select_data))
feature_data=[]
for data2 in select_data[0]:
    feature_data.append(parse_audio_files(data2))
...
for i in range(len(model)-1):
    ...
    df_fold=data_augment.create_df_k(k,feature_data,y_L_split)
